...
flags.DEFINE_string("train_file", None,
                    "KorQuAD json folder for training. E.g., train-v2.0")

flags.DEFINE_string(
    "predict_file", None,
    "KorQuAD json folder for predictions. E.g., dev-v2.0 or test-v2.0")
...
flags.DEFINE_bool(
    ...
    "If true, the KorQuAD examples contain some that do not have an answer.")
...
def read_squad_examples(input_file, is_training):
  """Read a KorQuAD json file into a list of KorquadExample."""
  ...
  for file_name in file_names:
    ...
    with tf.io.gfile.GFile(data_file,'r') as f:
        ...
        input_data.extend(dataset_json['data'])
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    tok_to_orig_index = [] # 토큰들의 doc_tokens에서의 원래 인덱스 
    orig_to_tok_index = [] # 토큰화된 인덱스
    all_doc_tokens = [] # tokenizer로 토큰화된 doc_token 토큰들 ex) I'm -> I,',m
    for (i, token) in enumerate(example.doc_tokens):
      orig_to_tok_index.append(len(all_doc_tokens))
      sub_tokens = tokenizer.tokenize(token) # doc_tokens 토큰화
      for sub_token in sub_tokens:
        tok_to_orig_index.append(i) # doc_token 인덱스 
        all_doc_tokens.append(sub_token) # sub_token 담음
    ...
    if is_training:
      ...
      # answer 토큰화해서 그에 맞는 새로운 start,end 포지션 리턴 
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings, use_gpu, num_gpu_cores):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      # create optimizer by using custom optimization code
      train_op = optimization_tf_multi.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps)

      # differentiate when we use gpu because we use different estimator
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder, batch_size):
  ...
  def input_fn(params):
    ...
    # if multi gpus, we use batch_size as parameter of input_fn_builde 
    d = tf.data.TFRecordDataset(input_file)

    if is_training:
      d = d.repeat()
      d = d.shuffle(buffer_size=100) # 100 to 500 하니까 loss 가 사라짐 
